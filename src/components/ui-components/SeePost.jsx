...
function SeePost() {
  ...
  useEffect(() => {
    async function fetchFile() {
      if (data.length > 0) {
        setLoading(true); // Start loader when fetching the file
        try {
          const currentFileId = data[currentIndex].fileId;
          let response = await storage.getFileView('66c595cc00017a8688f3', currentFileId);
          setVideoUrl(response.href);
          setLoading(false); // Stop loader once the video URL is ready
        } catch (e) {
          console.error("Error in fetching file", e);
          setLoading(false); // Stop loader in case of error
        }
  ...
  return (
    <div className='flex flex-col justify-center items-center'
      ...
      onTouchEnd={handleTouchEnd}
    >
      {loading ? ( // Conditionally show loader
        <div className="flex items-center justify-center h-full">
          <div className="loader border-t-4 border-blue-500 rounded-full w-12 h-12 animate-spin"></div>
          {/* A simple CSS spinner loader */}
        </div>
      ) : (
        data.length > 0 && videoUrl ? (
          <>
